...
const VolumeProfile: React.FC<VolumeProfileProps> = ({
  ...
  const { valueAreaHigh, valueAreaLow, poc } = useMemo(() => {
    ...
    const totalVolume = volumeProfile.reduce(
      (sum, node) => sum + node.totalVolume,
      0
    );
    ...
    while (
      ...
      (lowIndex > 0 || highIndex < volumeProfile.length - 1)
    ) {
      const lowVolume =
        lowIndex > 0 ? volumeProfile[lowIndex - 1].totalVolume : 0;
      const highVolume =
        highIndex < volumeProfile.length - 1
          ? volumeProfile[highIndex + 1].totalVolume
          : 0;
          ...
          <group key={index}>
            ...
            <mesh
              position={[
                x,
                0,
                -barLength * bidRatio - (barLength * (1 - bidRatio)) / 2,
              ]}
            >
      ...
      {poc && (
        <>
          ...
          <mesh position={[0, 0, -10]}>
            <boxGeometry
              args={[Math.abs(valueAreaHigh - valueAreaLow) * 0.02, 0.1, 20]}
            />
            <meshStandardMaterial color={0x6666ff} transparent opacity={0.2} />
          </mesh>
